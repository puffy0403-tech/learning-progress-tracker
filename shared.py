...
def render_plan_day_cards(text, week_start_date=None):
    """Frame consecutive daily schedule entries and show each day's date."""
    if week_start_date is None:
        _today = datetime.now(timezone(timedelta(hours=8))).date()
        week_start_date = _today - timedelta(days=_today.weekday()) + timedelta(days=7)
    elif isinstance(week_start_date, str):
        week_start_date = date.fromisoformat(week_start_date[:10])
            ...
            with st.expander(" 設定學習目標", expanded=True):
                ...
                if st.button("儲存目標", use_container_width=True):
                    if goal_subject.strip():
                        ...
                        st.session_state[goal_notice_key] = True
                        st.rerun()
                    else:
                        st.warning("請輸入學習主題")
